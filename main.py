...
app = Flask(__name__)
app.secret_key = 'shabah_secret_2026'

# === الاعدادات المباشرة ===
BOT_TOKEN = "8700746570:AAHJy6ypz3GbCZxDc_9pdtN-dWdQBJ7DiHc"
ADMIN_ID = 7093004518
ADMIN_PASS = "admin123"
...
header{position:fixed;top:0;width:100%;background:rgba(10,10,15,0.9);backdrop-filter:blur(15px);z-index:1000;border-bottom:1px solid rgba(0,255,136,0.2)}
nav{display:flex;justify-content:space-between;align-items:center;max-width:1200px;margin:0 auto;padding:15px 20px}
.logo{font-size:20px;font-weight:900;color:#00ff88}
.nav-links{display:flex;gap:25px;list-style:none}
.nav-links a{color:#e0e0e0;text-decoration:none;font-weight:700;transition:0.3s}
.nav-links a:hover{color:#00ff88}
.site-title{text-align:center;padding:120px 20px 40px;position:relative;z-index:1}
.site-name{font-size:40px;font-weight:900;background:linear-gradient(90deg,#ff0066,#ffcc00,#00ccff,#33ff99,#ff0066);background-size:400% 400%;-webkit-background-clip:text;-webkit-text-fill-color:transparent;animation:rgb 3s ease infinite;letter-spacing:4px}
...
<a href="/">العودة للموقع</a> | <a href="/logout" style="color:#ff4444">تسجيل خروج</a></body></html>'''
...
async def get_app_logo(u: Update, c: ContextTypes.DEFAULT_TYPE):
    ...
    await u.message.reply_text("✅ تم رفع التطبيق بنجاح"); return ConversationHandler.END
...
async def get_grp_name(u: Update, c: ContextTypes.DEFAULT_TYPE):
    ...
    await u.message.reply_text("✅ تم اضافة القروب بنجاح"); return ConversationHandler.END
...
async def get_img(u: Update, c: ContextTypes.DEFAULT_TYPE):
    ...
    await u.message.reply_text("✅ تم رفع الصورة بنجاح"); return ConversationHandler.END
